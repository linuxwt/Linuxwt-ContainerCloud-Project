...
For HA, please see [this repo](https://github.com/cmdicncf/charts/tree/master/cmdicncf/postgresql-ha)

## TL;DR;

```console
$ helm repo add cmdicncf https://charts.cmdicncf.com/cmdicncf
$ helm install my-release cmdicncf/postgresql
```

## Introduction

This chart bootstraps a [PostgreSQL](https://github.com/cmdicncf/cmdicncf-docker-postgresql) deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
| `image.registry`                              | PostgreSQL Image registry                                                                                                                                                 | `harbor.cmdicncf.org`                                                   |
| `image.repository`                            | PostgreSQL Image name                                                                                                                                                     | `cmdicncf/postgresql`                                          |
...
| `postgresqlDataDir`                           | PostgreSQL data dir folder                                                                                                                                                | `/cmdicncf/postgresql` (same value as persistence.mountPath)   |
...
| `persistence.mountPath`                       | Path to mount the volume at                                                                                                                                               | `/cmdicncf/postgresql`                                         |
...
| `metrics.image.registry`                      | PostgreSQL Exporter Image registry                                                                                                                                        | `harbor.cmdicncf.org`                                                   |
| `metrics.image.repository`                    | PostgreSQL Exporter Image name                                                                                                                                            | `cmdicncf/postgres-exporter`                                   |
...
```console
$ helm install my-release -f values.yaml cmdicncf/postgresql
```
...
To modify the PostgreSQL version used in this chart you can specify a [valid image tag](https://hub.docker.com/r/cmdicncf/postgresql/tags/) using the `image.tag` parameter. For example, `image.tag=12.0.0`
...
The [Bitnami PostgreSQL](https://github.com/cmdicncf/cmdicncf-docker-postgresql) image allows you to use your custom scripts to initialize a fresh instance. In order to execute the scripts, they must be located inside the chart folder `files/docker-entrypoint-initdb.d` so they can be consumed as a ConfigMap.
...
The [Bitnami PostgreSQL](https://github.com/cmdicncf/cmdicncf-docker-postgresql) image stores the PostgreSQL data and configurations at the `/cmdicncf/postgresql` path of the container.

Persistent Volume Claims are used to keep the data across deployments. This is known to work in GCE, AWS, and minikube.
See the [Parameters](#parameters) section to configure the PVC or to disable persistence.

If you already have data in it, you will fail to sync to standby nodes for all commits, details can refer to [code](https://github.com/cmdicncf/cmdicncf-docker-postgresql/blob/8725fe1d7d30ebe8d9a16e9175d05f7ad9260c93/9.6/debian-9/rootfs/libpostgresql.sh#L518-L556). If you need to use those data, please covert them to sql and import after `helm install` finished.
...
Subscribe to project updates by watching https://github.com/cmdicncf/cmdicncf-docker-postgresql
Submit issues and feature requests at https://github.com/cmdicncf/cmdicncf-docker-postgresql/issues
Send us your feedback at containers@cmdicncf.com

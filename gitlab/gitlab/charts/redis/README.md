...
$ helm repo add cmdicncf https://charts.cmdicncf.com/cmdicncf
$ helm install my-release cmdicncf/redis
```

```bash
# Production configuration
$ helm repo add cmdicncf https://charts.cmdicncf.com/cmdicncf
$ helm install my-release cmdicncf/redis --values values-production.yaml
```

## Introduction

This chart bootstraps a [Redis](https://github.com/cmdicncf/cmdicncf-docker-redis) deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
While [Redis Helm Chart](https://github.com/cmdicncf/charts/tree/master/cmdicncf/redis) will deploy a master-slave cluster using Redis Sentinel, the [Redis Cluster Helm Chart](https://github.com/cmdicncf/charts/tree/master/cmdicncf/redis-cluster) will deploy a Redis Cluster topology with sharding.
...
| `image.registry`                              | Redis Image registry                                                                                                                                | `harbor.cmdicncf.org`                                             |
| `image.repository`                            | Redis Image name                                                                                                                                    | `cmdicncf/redis`                                         |
...
| `metrics.image.registry`                      | Redis exporter image registry                                                                                                                       | `harbor.cmdicncf.org`                                             |
| `metrics.image.repository`                    | Redis exporter image name                                                                                                                           | `cmdicncf/redis-exporter`                                |
...
| `volumePermissions.image.registry`            | Init container volume-permissions image registry                                                                                                    | `harbor.cmdicncf.org`                                             |
| `volumePermissions.image.repository`          | Init container volume-permissions image name                                                                                                        | `cmdicncf/minideb`                                       |
...
| `sentinel.image.registry`                     | Redis Sentinel Image registry                                                                                                                       | `harbor.cmdicncf.org`                                             |
| `sentinel.image.repository`                   | Redis Sentinel Image name                                                                                                                           | `cmdicncf/redis-sentinel`                                |
...
| `sysctlImage.registry`                        | sysctlImage Init container registry                                                                                                                 | `harbor.cmdicncf.org`                                             |
| `sysctlImage.repository`                      | sysctlImage Init container name                                                                                                                     | `cmdicncf/minideb`                                       |
...
```bash
$ helm install my-release -f values.yaml cmdicncf/redis
```

> **Tip**: You can use the default [values.yaml](values.yaml)

> **Note for minikube users**: Current versions of minikube (v0.24.1 at the time of writing) provision `hostPath` persistent volumes that are only writable by root. Using chart defaults cause pod failure for the Redis pod as it attempts to write to the `/cmdicncf` directory. Consider installing Redis with `--set persistence.enabled=false`. See minikube issue [1990](https://github.com/kubernetes/minikube/issues/1990) for more information.

## Configuration and installation details

### [Rolling VS Immutable tags](https://docs.cmdicncf.com/containers/how-to/understand-rolling-tags-containers/)
...
To modify the Redis version used in this chart you can specify a [valid image tag](https://hub.docker.com/r/cmdicncf/redis/tags/) using the `image.tag` parameter. For example, `image.tag=X.Y.Z`. This approach is also applicable to other images like exporters.
...
```bash
$ helm install my-release --set persistence.existingClaim=PVC_NAME cmdicncf/redis
```
...
Find more information about how to deal with common errors related to Bitnami’s Helm charts in [this troubleshooting guide](https://docs.cmdicncf.com/general/how-to/troubleshoot-helm-chart-issues).
...
- Recommended: Create a clone of the Redis Master PVC (for example, using projects like [this one](https://github.com/edseymour/pvc-transfer)). Then launch a fresh release reusing this cloned PVC.
   ...
   ```
   helm install my-release cmdicncf/redis --set persistence.existingClaim=<NEW PVC>
   ```
...
- Alternative (not recommended, do at your own risk): `helm delete --purge` does not remove the PVC assigned to the Redis Master StatefulSet. As a consequence, the following commands can be done to upgrade the release
   ...
   helm install <RELEASE> cmdicncf/redis
   ```
...
The metrics exporter has been changed from a separate deployment to a sidecar container, due to the latest changes in the Redis exporter code. Check the [official page](https://github.com/oliver006/redis_exporter/) for more information. The metrics container image was changed from oliver006/redis_exporter to cmdicncf/redis-exporter (Bitnami's maintained package of oliver006/redis_exporter).

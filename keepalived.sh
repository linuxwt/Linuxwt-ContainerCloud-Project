...
ip=$(ip addr | grep ens160 | grep inet | awk '{print $2}' | awk -F '/' '{print $1}')
...
if [ $ip == "192.168.0.113" ];then
    sed -i 's/192.168.0.112/192.168.0.113/g' /etc/keepalived/keepalived.conf
    ...
    sed -i 's/192.168.0.112/192.168.0.114/g' /etc/keepalived/keepalived.conf
